...
export default function IntroducingGirionixPage({ isOpen, onClose, onLaunchApp, onOpenDownload, initialTab = 'overview' }) {
  ...
  const comparisons = [
    { feature: 'Universal Multi-Studio (Code, Screenplay, Math, 8K Art, 4K Video, Audio)', girionix: '✅ 6 Native Integrated Studios', others: '❌ Fragmented separate subscriptions ($100+/mo)' },
    ...
    { feature: 'Cost & Mandatory Subscriptions', girionix: '✅ 100% Free & Open Access', others: '❌ $20 to $200/month recurring fees' }
  ];
      ...
      <nav className="h-16 px-4 sm:px-8 flex items-center justify-between border-b border-white/10 bg-[#07080E]/90 sticky top-0 z-50 backdrop-blur-xl">
        ...
        <div className="hidden sm:flex items-center gap-1.5 p-1 rounded-2xl bg-white/5 border border-white/10 text-xs font-mono">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl transition-all ${activeTab === 'overview' ? 'bg-cyan-500/20 text-cyan-300 font-bold border border-cyan-500/40' : 'text-gray-400 hover:text-white'}`}
          >
            Overview & Studios
          </button>
          <button
            onClick={() => setActiveTab('comparison')}
            className={`px-3 py-1.5 rounded-xl transition-all ${activeTab === 'comparison' ? 'bg-purple-500/20 text-purple-300 font-bold border border-purple-500/40' : 'text-gray-400 hover:text-white'}`}
          >
            Why Switch?
          </button>
          <button
            onClick={() => setActiveTab('downloads')}
            className={`px-3 py-1.5 rounded-xl transition-all ${activeTab === 'downloads' ? 'bg-emerald-500/20 text-emerald-300 font-bold border border-emerald-500/40' : 'text-gray-400 hover:text-white'}`}
          >
            📦 Standalone Apps
          </button>
        </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {[
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-mono font-bold transition-all cursor-pointer ${activeStudio === tab.id ? 'bg-white/15 text-white border border-cyan-400 shadow-glow-cyan' : 'bg-white/5 text-gray-400 hover:text-white border border-white/10'}`}
              >
                {tab.label}
              </button>
            ))}
          ...
          <div className="space-y-2 max-w-3xl mx-auto">
            {faqs.map((faq, i) => (
              <div key={i} className="rounded-2xl bg-[#090C1A] border border-white/10 overflow-hidden">
                ...
                >
                  <span>{faq.q}</span>
                  <ChevronRight className={`w-4 h-4 text-cyan-400 transition-transform ${openFaq === i ? 'rotate-90' : ''}`} />
                </button>
                {openFaq === i && (
