...
export default function ScratchpadModal({ isOpen, onClose, activeModel }) {
  const [content, setContent] = useState(() => {
    return localStorage.getItem('abyntra_scratchpad_notes') || '# Abyntra Quick Scratchpad\n\n- Jot down ideas, prompts, or snippets here\n- Automatically saved in real-time\n';
  });
  ...
  useEffect(() => {
    localStorage.setItem('abyntra_scratchpad_notes', content);
  }, [content]);
  ...
  const handleDownload = () => {
    ...
    a.download = `Abyntra_Notes_${Date.now()}.md`;
  ...
  const handleAiSummarize = async () => {
    ...
    try {
      ...
      await openrouter.streamChat({
        messages: [
          { role: 'system', content: 'You are Abyntra AI Executive Editor. Format and polish these notes into clean, structured Markdown bullet points with key insights.' },
          { role: 'user', content }
        ],
